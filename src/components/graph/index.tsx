...
export default class Graph extends React.Component<Props, {}> {
  ...
  componentDidMount() {
    console.log("Graph Data:", this.props.tree);

    this.networkChart = this.renderChartCollapsibleNetwork()
      .svgHeight(window.innerHeight - 30)
      .svgWidth(window.innerWidth - 30)
      .container("#app")
      .data({ root: this.props.tree })
      .debug(true)
      .run();
  }
  ...
  renderChartCollapsibleNetwork = (params?: any) => {
    // exposed letiables
    let attrs = {
    ...
    let main = function(selection) {
      selection.each(function scope() {
        ...
        //link wrapper
        let linksWrapper = chart.patternify({
          tag: "g",
          selector: "links-wrapper"
        });

        //node wrapper
        let nodesWrapper = chart.patternify({
          tag: "g",
          selector: "nodes-wrapper"
        });
        let nodes, links, enteredNodes;
        ...
        function update(clickedNode) {
          ...
          let exited = nodes.exit().remove();
          let enteredNodes = nodes
            .enter()
            .append("g")
            .attr("class", "node");
          ...
          let channelsGrandchildren = enteredNodes
            .append("circle")
            .attr("r", 7)
            .attr("stroke-width", 5)
        ...
        function nodeMouseEnter(d) {
          ...
          //get links
          let links = hierarchy.root.links();
        ...
        function debug() {
          if (attrs.isDebug) {
            //stringify func
            let stringified = scope + "";

            // parse letiable names
            ...
            //assign local letiables to the scope
    ...
    Object.keys(attrs).forEach(key => {
      // Attach letiables to main function
